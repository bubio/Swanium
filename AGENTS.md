...
This file provides guidance to AI coding agents (Claude Code, Codex, etc.) working with code in this repository.

## Project status

Phase 1 (V30MZ CPU core) of `docs/dev/DevelopmentPlan.md` is substantially complete; Phase 7 (GUI) is in progress. See `docs/dev/Status.md` for the detailed, up-to-date implementation inventory (implemented instructions, deferred features, frontend progress). Update `docs/dev/Status.md` — not this file — when progress changes.
...
├── docs/
│   └── dev/        # Blueprint.md (vision), DevelopmentPlan.md (roadmap), Status.md (implementation status)
└── tests/
