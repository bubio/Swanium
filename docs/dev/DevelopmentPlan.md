...
> 現在の進捗確認は [Status.md](./Status.md) を見る。この文書は初期ロードマップと
> 設計メモを残すためのもので、最新の完了/未完了一覧ではない。ドキュメント全体の
> 役割分担は [README.md](./README.md) を参照。
...
**現在の扱い**
-   Phase 3 の初期目標は完了済み。自作マシンコード統合テストに加えて、
...
## 10.4 公開 ROM テストの pass/fail oracle

`crates/core/tests/public_roms.rs` の初期プレースホルダー判定は解決済み。
WSCPUTest は画面出力の `Ok!` / `Failed!` をデコードし、`ws-test-suite` は
`mono/cpu/80186_quirks.ws` のソース確認済み pass/fail marker を oracle として使う。
