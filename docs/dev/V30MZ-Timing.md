...
- **`N+`（分岐）**: stsws は条件分岐/ループを `N+` と記す。
  `N` = 分岐**不成立**時のクロック。分岐**成立**時はプリフェッチキュー再充填が入り、
  近ジャンプ相当（stsws で近 `JMP`=4）になる。本実装では
  **不成立 = 基準値、成立 = 4（`Jcc`）/ 基準 + 3（ループ）** と解釈している。
...
| `LOOP` (DBNZ) | E2 | 2 / 5 | 不成立 / 成立 |
| `LOOPE` (DBNZE) | E1 | 3 / 6 | 不成立 / 成立 |
| `LOOPNE` (DBNZNE) | E0 | 3 / 6 | 不成立 / 成立 |
| `JCXZ` | E3 | 1 / 4 | 不成立 / 成立 |
...
| 命令 | オペコード | 値 |
|---|---|--:|
| `IN` / `OUT acc, imm8` | E4–E7 | 6 |
| `IN` / `OUT acc, DX` | EC–EF | 6 |
...
5. **分岐成立ペナルティ**: `N+` の解釈として成立時 `Jcc`=4 / ループ=基準+3 を採用。
   プリフェッチキュー深さに依存する厳密値ではない近似。
