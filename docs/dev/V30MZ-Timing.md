...
- **公開 ROM 実測**: FluBBaOfWard/WSTimingTest v0.4.0。Pink
  ...
  では pages 0-28 を opt-in oracle として追加し、主要 opcode group、
  addressing variants、REP/string I/O、exception/interrupt timing、loop edge を
  照合した。
...
- **`N+`（分岐）**: stsws は条件分岐/ループを `N+` と記す。
  `N` = 分岐**不成立**時のクロック。WSTimingTest oracle に合わせ、
...
| `HALT` | F4 | 9 | |
| `NOP` | 90 | 1 | WSTimingTest confirmed |
...
5. **分岐成立ペナルティ**: WSTimingTest により、taken `Jcc` は 5、
   奇数アドレスへの分岐は +1、taken `LOOP` は 6 としている。call/jump 系も
   pages 0-28 の範囲で現行の1命令総クロック粒度を照合している。
